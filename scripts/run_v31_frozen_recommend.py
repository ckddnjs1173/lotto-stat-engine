...
from lotto_engine.v31_frozen_recommendation import generate_frozen_recommendations
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Frozen v3.1 personal-model recommendation calculation."
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("data/cache/v31_frozen_recommendation.json"),
    )
    args = parser.parse_args()

    try:
        payload = generate_frozen_recommendations(
    ...
    meta = payload["meta"]
    print("=" * 100)
    print("LOTTO STAT ENGINE v3.1 - FROZEN PERSONAL MODEL")
    print("POST-SELECTION EXPLORATORY VALIDATION; NO PROVEN LOTTERY EDGE CLAIM")
    print("=" * 100)
    ...
    print(
        f"reference={meta['training']['reference_samples_per_target']} nested stream0; "
        f"negatives={meta['training']['train_negatives_per_target']} nested stream0; "
    ...
    print(f"frozen recommendation JSON saved: {saved}")
