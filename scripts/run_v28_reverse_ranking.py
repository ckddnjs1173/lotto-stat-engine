...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="v2.8 strict nested reverse-learning combination ranking audit"
    )
    parser.add_argument("--start-index", type=int, default=BACKTEST_START_INDEX)
    parser.add_argument(
        "--min-meta-train-targets",
        type=int,
        default=MIN_META_TRAIN_TARGETS,
    )
    parser.add_argument(
        "--train-negatives-per-target",
        type=int,
        default=TRAIN_NEGATIVES_PER_TARGET,
    )
    parser.add_argument(
        "--baseline-samples",
        type=int,
    ...
    df = load_lotto_data()
    payload = run_v28_reverse_ranking_audit(
        df,
    ...
    print("=" * 104)
    print("V2.8 REVERSE-LEARNING NESTED RANKING AUDIT")
    print("=" * 104)
    ...
    print("latest learned model for next draw (diagnostic only)")
    print(
        f"  solved_targets={latest['solved_targets']} "
        f"pair_rows={latest['pair_rows']} ridge_lambda={latest['ridge_lambda']}"
    )
    for name, values in latest["features"].items():
        print(
    ...
    print(
        f"  - survivor => frozen confirmation with {CONFIRMATION_BASELINE_SAMPLES:,} "
        "fair candidates / outer target"
    )
    print("  - failure => no lambda/feature/horizon retuning from this result")
