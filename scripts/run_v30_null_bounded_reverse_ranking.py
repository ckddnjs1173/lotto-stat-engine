...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="v3.0 strict nested fair-null bounded quadratic reverse-ranking audit"
    )
    ...
    parser.add_argument(
        "--reference-samples", type=int, default=REFERENCE_SAMPLES_PER_TARGET
    )
    ...
    df = load_lotto_data()
    payload = run_v30_null_bounded_reverse_ranking_audit(
        df,
    ...
    print("=" * 112)
    print("V3.0 FAIR-NULL BOUNDED QUADRATIC REVERSE-LEARNING NESTED RANKING AUDIT")
    print("=" * 112)
    ...
    print(f"fair reference samples / target: {payload['reference_samples_per_target']}")
    print(f"ridge lambda: {payload['ridge_lambda']}")
    print(f"bounded quadratic basis width: {payload['basis_width']}")
    ...
    print("raw feature direction: preserved monotonically through fair-null midrank")
    print("all transformed base coordinates: bounded to [-1, 1]")
    print("all quadratic basis values: bounded to [-1, 1]")
    print()
    ...
    print("latest bounded quadratic model (top 15 |scaled weight|)")
    print(
    ...
    print("  - this test does not reward balanced/pretty number shapes by rule")
    print("  - extreme values remain eligible; they simply cannot gain score from raw magnitude explosion")
    print("  - validation remains separate from personal next-draw raw output")
    print("  - screening gate remains mean>50, CI lower>0, recent300>=50, recent100>=50")
    print(
        f"  - survivor => frozen {CONFIRMATION_BASELINE_SAMPLES:,}-candidate confirmation before any "
        "replacement of the current personal raw path"
    )
