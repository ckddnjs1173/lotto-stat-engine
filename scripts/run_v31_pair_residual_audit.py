...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="v3.1 pair vertex-main-effect residualization walk-forward audit"
    )
    ...
    parser.add_argument(
        "--output-json",
        type=Path,
        default=Path("data/cache/v31_pair_residual_audit.json"),
    )
    args = parser.parse_args()

    try:
        payload = run_pair_residualization_audit(
            load_lotto_data(),
        ...
        print(f"pair residual audit failed: {exc}")
        raise SystemExit(1) from exc

    print("=" * 100)
    print("V3.1 PAIR RESIDUALIZATION AUDIT - DIAGNOSTIC ONLY")
    print("=" * 100)
    ...
    delta = payload["pair_residualized_minus_original"]
    print(
        f"residual-original: mean={delta['mean_percentile_delta']:+.4f} "
    ...
    for key, item in payload["pair_vertex_projection"].items():
        print(
            f"{key}: mean vertex-explained={item['mean_explained_variance_fraction']:.4f} "
            f"latest={item['latest']['explained_variance_fraction']:.4f}"
        )
